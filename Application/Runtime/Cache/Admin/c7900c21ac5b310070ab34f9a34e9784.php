...
<div class="content">
    ...
    <div class="well">
        <!-- edit form -->
        <form action="/index.php/Admin/Manager/edit/id/3" method="post" id="tab">
            <input type="hidden" name="id" value="<?php echo ($manager["id"]); ?>"></input>

          <label>角色:</label>
            <select name="role_id"  >
                <?php if(is_array($role)): $k = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($k % 2 );++$k;?><option value="<?php echo ($vol["role_id"]); ?>" <?php if( $vol["role_id"] == $manager["role_id"] ): ?>selected="selected"<?php endif; ?> > <?php echo ($vol["role_name"]); ?> </option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>

            <label>用户名：</label>
            <input type="text" name="username" value="<?php echo ($manager["username"]); ?>" class="input-xlarge" disabled="true">
            <label>昵称：</label>
            <input type="text" name="nickname" value="<?php echo ($manager["nickname"]); ?>" class="input-xlarge">
            <label>邮箱：</label>
            <input type="text" name="email" value="<?php echo ($manager["email"]); ?>" class="input-xlarge">
