...
<div class="content">
    ...
    <div class="well">
        <!-- add form -->
        <form action="/index.php/Admin/Manager/add" method="post" id="tab">
            <label>角色:</label>
            <select name="role_id"  >
                <?php if(is_array($role)): $k = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($k % 2 );++$k;?><option value="<?php echo ($vol["role_id"]); ?>"><?php echo ($vol["role_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
            <label>用户名：</label>
