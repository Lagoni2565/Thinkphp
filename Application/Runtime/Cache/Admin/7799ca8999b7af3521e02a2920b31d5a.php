...
<div class="content">
    ...
    <div class="well">
    <a class="btn btn-primary" onclick="javascript:window.location.href='/index.php/Admin/Attribute/add'">新增</a>
        <!-- table -->
        <table class="table table-bordered table-hover table-condensed">
            ...
            <tbody>
            <?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr class="success">
                    <td><?php echo ($vol["attr_id"]); ?></td>
                    <td><?php echo ($vol["attr_name"]); ?></td>
                    <td><?php echo ($vol["type_name"]); ?></td>
                    <td><?php if( $vol["attr_type"] == 0 ): ?>唯一属性<?php else: ?>单选属性<?php endif; ?></td>
                    <td>
                        <?php if( $vol["attr_input_type"] == 0 ): ?>输入框
                        <?php elseif( $vol["attr_input_type"] == 1 ): ?>下拉列表
                        <?php else: ?>多选框<?php endif; ?>
                    </td>
                    <td><?php echo ($vol["attr_values"]); ?></td>
                    <td>
                        <a href="/index.php/Admin/Attribute/edit/attr_id/<?php echo ($vol["attr_id"]); ?>"> 编辑 </a>
                        <a href="javascript:void(0);" onclick="if(confirm('确认删除？')) location.href='/index.php/Admin/Attribute/delete/attr_id/<?php echo ($vol["attr_id"]); ?>'"> 删除 </a>
                    </td>
