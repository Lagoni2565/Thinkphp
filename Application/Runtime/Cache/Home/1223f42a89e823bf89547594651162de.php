				...
				<div class="clearfixLeft" id="clearcontent">
					...
					<div class="box">
						...
						<ul class="tb-thumb" id="thumblist">
							<?php if(is_array($goods_pics)): $k = 0; $__LIST__ = $goods_pics;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($k % 2 );++$k;?><li class="<?php if( $k == 1 ): ?>tb-selected<?php endif; ?>">
								<div class="tb-pic tb-s40">
									<a href="javascript:void(0);"><img src="<?php echo ($vol["pics_sma"]); ?>" mid="<?php echo ($vol["pics_mid"]); ?>" big="<?php echo ($vol["pics_big"]); ?>"></a>
								</div>
							...
							<dd>
								...
								<div class="theme-popover">
									...
									<div class="theme-popbod dform">
										<form class="theme-signin" name="goodsform" action="/index.php/Home/Cart/addcart" method="post">
											<input type="hidden" name="goods_id" value="<?php echo ($goods["id"]); ?>">
											<div class="theme-signin-left">
												<!--遍历输出商品属性-->
												<?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_attr): $mod = ($i % 2 );++$i; if($vol_attr["attr_type"] == 1 ): ?><div class="theme-options goods_attr">
													<div class="cart-title"><?php echo ($vol_attr["attr_name"]); ?></div>
													<ul>
														<?php if(is_array($goods_attr)): $i = 0; $__LIST__ = $goods_attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_goods): $mod = ($i % 2 );++$i; if( $vol_goods["attr_id"] == $vol_attr["attr_id"] ): ?><li class="sku-line" goods_attr_id="<?php echo ($vol_goods["id"]); ?>"><?php echo ($vol_goods["attr_value"]); ?><i></i></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
													</ul>
						...
						<div class="am-tabs-bd">
							...
							<div class="am-tab-panel am-fade am-in am-active">
								<div class="J_Brand">
									...
									<div class="clear"></div>
									<ul id="J_AttrUL">
										<?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_attr): $mod = ($i % 2 );++$i; if( $vol_attr["attr_type"] == 0 ): ?><li title=""><?php echo ($vol_attr["attr_name"]); ?>:&nbsp;
											<?php if(is_array($goods_attr)): $i = 0; $__LIST__ = $goods_attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol_goods): $mod = ($i % 2 );++$i; if($vol_goods["attr_id"] == $vol_attr["attr_id"] ): echo ($vol_goods["attr_value"]); endif; endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
	$(function(){
		...
		$('#LikBasket').click(function(){
			//判断属性是否都选择了对应的属性值 通过个数对比
			//获取单选属性名称的个数
			var num = $('.goods_attr').length;
			//获取选中的属性值的个数
			//获取用户选择的单选属性值对应的主键id,拼接成goods_attr_ids 2,3,
			var selected_li = $('.sku-line.selected');
			if(num != selected_li.length){
				alert('必须选择属性');
				return false;
			}
			var goods_attr_ids = '';
			$.each(selected_li, function(i,v){
				//v就是一个li标签，是一个document对象
